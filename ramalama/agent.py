...
class AgentEngineArgsType(BaseEngineArgsType):
    ARGS: list[str]
    agent_image: str
    workdir: str | None
...
class AgentEngine(Engine):
    """Engine for running agent containers."""

    def __init__(self, args: AgentEngineArgsType, *, model_name: str) -> None:
    ...
    def is_tty_cmd(self) -> bool:
        return getattr(self.args, "subcommand", "") == "agent"
...
class GooseEngine(AgentEngine):
    """Goose-specific agent engine.

    Extends AgentEngine with environment variables required by Goose, and
    ...
    def __init__(self, args: AgentEngineArgsType, *, model_name: str) -> None:
        ...
        self.add_name(f"agent-{self.args.name}")  # type: ignore[attr-defined]
        self.add_workdir()
        self.add_args(args.agent_image)
        if args.ARGS:
...
def run_agent(args):
    """Orchestrate model server and agent containers."""
    ...
    runtime = get_runtime(get_config().runtime)
