...
## SYNOPSIS
**ramalama agent** [*options*] *model* [arg ...]
...
When run with no arguments after the model, an interactive Goose session is
launched. If one or more arguments are provided, they are passed to Goose as
instructions to process non-interactively. Commands may also be passed via
stdin.
...
```
ramalama agent qwen3:1.7b
```

Run the agent with a custom agent image:
```
ramalama agent qwen3:1.7b --agent-image ghcr.io/block/goose:1.27.0
```

Turn off thinking mode in the model the agent is connecting to (may result in faster responses):
```
ramalama agent qwen3:1.7b --thinking=off
```

Start an interactive session with access to a local directory:
```
ramalama agent qwen3:1.7b -w ./src
```

Request the agent to perform actions non-interactively:
```
ramalama agent qwen3:1.7b -w ./src Please analyze the source code in the current directory
```

Send instructions to the agent via stdin:
```
echo "What is the speed of light in meters per second?" | ramalama agent qwen3:1.7b
```
