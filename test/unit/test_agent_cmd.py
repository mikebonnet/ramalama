...
def test_agent_model_positional():
    """Agent cli should accept a model as a positional argument"""
    _, args = parse_args_from_cmd(["agent", TEST_MODEL])
...
def test_agent_custom_agent_image():
    ...
    _, args = parse_args_from_cmd(["agent", TEST_MODEL, "--agent-image", "myimage:v1"])
...
def test_agent_workdir_option():
    ...
    _, args = parse_args_from_cmd(["agent", TEST_MODEL, "-w", "/tmp"])
    assert args.workdir == "/tmp"

    _, args = parse_args_from_cmd(["agent", TEST_MODEL, "--workdir", "/tmp"])
    assert args.workdir == "/tmp"
